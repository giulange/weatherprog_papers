...
\begin{enumerate}
    ...
    \begin{figure}[!t] % [H] ensures the figure appears exactly here
    	\centering % trim=left bottom right top
    	\includegraphics[angle=0,scale=.45,trim=0cm 0cm 0cm 0cm,clip]{Fig05_Box01_WeatherProg_concreteOperations.png}
...
\begin{figure}[th]
	\centering
    \includegraphics[scale=0.4]{Fig08_boxplot_perturbation.png}
...
%To this regard. the Net Gain Ratio is introduced as \( \frac{\Delta(\text{TP} + \text{TN})}{\Delta(\text{FP} + \text{FN})} \) to quantify the trade-off between the increase in correctly and incorrectly classified cases as model refinements are applied. It expresses how many additional correctly classified data points are obtained for each new misclassification introduced.
...
%\statusblock{Q10R3}{review}{Similar results were obtained by \citet{QC-temperature-ML-2024} with 88,9\% of true positives implementing their modular rule-based system.}
...
While their approach relies on real-world labels, our perturbation-based validation allows fine-grained, point-level evaluation. % and quantifies detection performance across a range of artificial disturbances.
...
%To further position these results in context, it is worth comparing them with recent literature. For instance, \citet{QC-temperature-ML-2024} evaluated a rule-based quality control system for agricultural weather stations, reporting an event-level recall of 91\% based on expert-labelled data. Unlike that study, our approach uses synthetic perturbation, which allows for a more detailed and controlled assessment of the detection accuracy at the level of each data point. A similar strategy was used by Good et al. (2022) in the oceanographic domain, where known errors were artificially added to evaluate automatic QC systems, achieving true positive rates between 86\% and 94\%, depending on the error type.
%Our method differs from event-based validation approaches by offering precise detection metrics for each perturbed value, which is especially useful in operational settings where fine-grained control and reproducibility are needed. Moreover, while our main focus is on anomaly detection, the validation strategy is conceptually related to recent studies on data reconstruction in environmental monitoring, such as those by Xie et al. (2021) and Lalic et al. (2024), who applied advanced machine learning methods to fill gaps in temperature records, reaching RMSEs below 0.5\si{\degreeCelsius}. However, unlike those studies, our system is designed not only to reconstruct data but also to quantify detection performance under different stress scenarios.
%This comparison highlights the novelty of our work in both the method used for validation and the type of performance indicators produced, showing that a perturbation-based strategy can offer a reliable and reproducible way to benchmark quality control systems in agrometeorology.
...
%\statusblock{Q10R3}{review}{Comparative studies in the literature confirm the strong performance of WeatherProg, such as the work by \citet{gap-filling-lalic2024}, which reports RMSE values exceeding 1.1.}
%\statusblock{Q10R3}{review}{

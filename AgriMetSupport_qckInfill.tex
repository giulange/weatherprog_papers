...
An interoperable and geospatial Cyber-Physical System (CPS) -- called Agri\-Met\-Support -- was implemented to manage sensor networks, weather stations, agrometeorological data, and services that can be developed sequentially (e.g., models enabling territorial and integrated risk management).
...
Agri\-Met\-Support is made by physical, software and modular components which are all specifically designed to build an operative cyber-physical system supporting responsable entities in their management and planning tasks, but also any other stakeholder dealing with the production or the consumption of either sensor data or any kind of service based on these data.
The fundamental aim of the work proposed in this paper is to validate the production of qualified and certified data using well-tuned procedures available along with Agri\-Met\-Support.
...
\statusblock{}{wip}{NOTES: (1) check that at crucial locations in the manuscript I highlight the distinct feature of \textit{geospatial} for my CPS.}
...
To address these challenges, a comprehensive solution through the development of a modular and interoperable Cyber-Physical System (CPS) is proposed.

% Objectives:
To achieve this goal, the present study focuses on the following objectives:
\begin{enumerate}
    \item Develop a \statusblock{}{wip}{geospatial(AMS pros)}, modular and interoperable CPS, called Agri\-Met\-Support, that supports the  management, dissemination and use of agrometeorological data (\cref{sec:gci} and \cref{sec:weatherprog}).
    ...
    \item Estabilish that AgriMetSupport is valuable in research and science (\cref{sec:Valuability})\\
\end{enumerate}
...
Section \ref{sec:qck+fill} provides details about the data quality control and the gap-filling procedures, highlighting the operations required to produce data of good quality \statusblock{}{wip}{(REMOVE?)to be used in cascade in other applications}.
...
\statusblock{}{wip}{REMOVE OR MOVE: The main finding is that such a cyber-physical system can be spread to distinct territories (or distinct networks within the same territory) or used as a centralized system to manage federated monitoring networks.
It can ease the management of sensor networks by responsible entities while leaving complex issues to researchers or domain experts.}
...
Agri\-Met\-Support is designed to be a comprehensive framework for agrometereological monitoring and data management.
...
Clearly, different real-world applications have different requirements, which can be all accomodated by Agri\-Met\-Support thanks to its strongly modular architecture.
Components can be added or removed to a specific instance of Agri\-Met\-Support effortlessly without affecting other parts of the system, as well as modules of the core engine, namely WeatherProg, which can be selectively enabled or disabled.
This way, responsible entities can actually derive their own tailored instances from the general model of Agri\-Met\-Support.
...
\statusblock{reply to ..., Q7R1}{review}{%Interoperability.\\
...
The Agri\-Met\-Support infrastructure is based on the distinction amongst data, logic and visualisation tiers (\cref{Fig_tiers}).
...
\begin{enumerate}
    ...
    \begin{figure}[!t] % [H] ensures the figure appears exactly here
    	...
    	\caption{NOTES: Change the black map in point (9); CAPTION: Concrete examples about WeatherProg modules.}
...
\statusblock{}{wip}{REVIEW THIS SECTION HAVING IN MIND THE NEW FIGURES, SPECIALLY \cref{Box01:WeatherProg_Operations}.}
...
\statusblock{reply to Q6R3.2}{done}{A user of Agri\-Met\-Support should always be aware of the level of confidence and reliability associated with the use of each internal component, with particular emphasis here on WeatherProg. % and its constituent modules as they were shown in \cref{sec:weatherprog}.
}
...
\statusblock{delete this comment}{done}{any reference to infill is not yet written here. Currently, I decide that no reference to infill must be provided here since it is shown in results. Here we only have the part related to the building of the validation procedure.}
...
\statusblock{}{review}{The quality of the AgriMetSupport system is evaluated based on several performance criteria, excluding assessments related to the system as a whole, such as decision support, user interaction efficiency, data dissemination effectiveness, and integration with external services.
...
AgriMetSupport exemplifies a model \statusblock{}{wip}{geospatial CPS}  that can be seamlessly customised to manage sensor networks across several territories.
...
\statusblock{reply to Q6R3.2}{done}{As delineated in the quality control logic of WeatherProg, %(\cref{sec:qcheck}), 
...
%     \end{itemize}
% \end{itemize}
\statusblock{answer to \textbf{Q5R2}}{review}{NOTES: I have to highlight two distinct things:\\
...
% \end{itemize}
\statusblock{}{wip}{The scientific contribution of AgriMetSupport lies in its innovative validation methodology, which is based on the perturbation strategy described above for validating quality control and gap-filling methods.
...
Together, these methods enable the quantification of errors %in WeatherProg’s quality control and gap-filling algorithms, 
...
\statusblock{Q5R1}{review}{The presented statistical results demonstrate the robustness and effectiveness of the quality control and gap-filling methodologies, directly enhancing the reliability of agricultural meteorological datasets. 
...
In conclusion, Agri\-Met\-Support embodies a significant step forward in the search for a more integrated, efficient, and user-friendly approach to agrometeorological and sensor networks monitoring.
...
In addition to the demonstrated capabilities and potential future directions previously outlined, Agri\-Met\-Support is ready to play a role in the ongoing Italian Agritech project. 
...
Central to its contribution to the Agritech project, Agri\-Met\-Support's ability in generating high-resolution raster maps of critical agrometeorological variables is significantly enhanced through the essential use of quality-controlled data.
...
By leveraging data that have undergone rigorous quality control, as detailed in the results of this study, Agri\-Met\-Support can substantially improve the accuracy and effectiveness of pest simulation models. 

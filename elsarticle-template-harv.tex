...
\title{ Qualified data production for agricultural meteorology management and planning }
...
A data consulting web application publishes the agroclimatic data managed by the system to the general public, whereas more tailored services are provided through dedicated interfaces.
...
\begin{enumerate}
    ...
    An anomalous datum is detected thanks to a multilevel technology based on interlinked and different kinds of quality checks such as range, continuity, climatological, spatial, temporal and persistency checks.
...
\section{Create qualified (certified) data with no gaps}
...
\subsection{Quality control procedure \review{(maybe it's better to call it quality control everywhere)}}[Giuliano][review] \label{sec:qcheck}
...
For simplicity, an overview of the quality control procedure (\cref{tab:qcheck_m10_airT}) is presented only at the 10 minutes time scale for air temperature both in this section and in section \ref{QcheckValidation} where the quality control procedure is validated.
There is a logic based on 15 different combinations of four quality control types, namely range, continuity, persistence and spatial.
Each combination shown in \cref{tab:qcheck_m10_airT} can be reached only if all previous combinations are not met while the sequence of combinations depends on the absolute and relative weight given to each control type and also accounts for the desired outcome status.
...
The descriptions of flags for the outcome of the quality control procedure can be found in \cref{tab:qcheckOutcomeFlagsSummary}.
...
\subsection{Infill gaps procedure}
\note{Describe here the mlr proc that will be used in validation study later in the paper.}
%Anomalies and missing data are all flagged to be %interpolated by means of an automatic linear regression procedure.
%infilled in order to get complete and homogeneous time series.
%Different methods are available such as a deterministic approach based on moving average with a growing kernel, or a statistical method approach using a stepwise multilinear regression using other stations after an iterative optimization step to properly select the time series length and the regression covariates.

WeatherProg has a module dedicated to infill anomalies and gaps in the data.
For the purpose of this paper, the stepwise multilinear regression infilling type was used in order to allow for a validation analysis.
The logic of the infill module is based on the selection of all gaps and anomalies for each station. %given a reference interval of time.
Then, the infill module runs the statistical interpolation block which includes an iterative optimisation guiding the selection of both the time series length and the number of regression covariates (i.e. other stations used as independent variables).
The model characterised by the lowest RMSE is selected to predicted the infilled value for that station in that date and time.
...
The infill module takes care to avoid the selection of auxiliary singletons that couldn't pass the quality check procedure (i.e. flags that are different from CORRECT or DUBIOUS).
...
If the perturbation switch is active, noisy data are created according to different levels of absolute amplitude ($A$) and quota ($Q$), where amplitude is a magnitude exaggeration factor applied to air temperature ($A=10$ in this validation study) and quota accounts for the relative amount of raw measurements in which insert noise ($Q=30\%$ in this validation study).
...
In \cref{fig:perturbationCharts} it is shown the evolution of the quality control thanks to the feedback received by the results of validation study.

\note{add: (1) rmse on qcheck; (2) rmse on infilled data}\\
Need to show results of the perturbation study.
